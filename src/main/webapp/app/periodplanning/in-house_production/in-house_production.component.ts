...
export class InHouseProductionComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.teilService.query()
            .subscribe((res: ResponseWrapper) => {
                ...
                const teiltypen = teil.teiltyp.toString();
            ...
            this.wantedfewartend = this.fertigungsauftraege.filter(function(fertigungsauftragwartend){
                ...
                const auftragsstatuse = fertigungsauftragwartend.auftragsstatus.toString();
            ...
            this.wantedfebearbeitend = this.fertigungsauftraege.filter(function(fertigungsauftragbearbeitend){
                ...
                const auftragsstatuse = fertigungsauftragbearbeitend.auftragsstatus.toString();
                // console.log(auftragsstatuse);
            ...
            for (let i = 0; i < this.inhouse_anzeige_array.length; i++ ) {
                console.log('Meine Anzeige: + Nummer: ' + this.inhouse_anzeige_array[i].nummer + '  VT-Wunsch: '
                    + this.inhouse_anzeige_array[i].vertriebswunsch + ' Sicherheitsbestand:  ' + this.inhouse_anzeige_array[i].sicherheitsbestand +
                    + ' bestand_vorperiode:  ' + this.inhouse_anzeige_array[i].bestand_vorperiode +  ' auftraege_in_warteliste:  '
                    + this.inhouse_anzeige_array[i].auftraege_in_warteliste +
                    ' auftraege_in_bearbeitung:  ' + this.inhouse_anzeige_array[i].auftraege_in_bearbeitung
                    + ' produktionsauftraege:  ' + this.inhouse_anzeige_array[i].produktionsauftraege
                    );
            }
    ...
    public anzahl_fertigungsauftraege_zu_herstellteil_warteliste(wanted: Teil[], wantedfewartend: Fertigungsauftrag[]) {
        ...
        const array_warteliste_eines_teils_zu_herstellteilmenge = new Array;
        let warteliste_eines_teils_zu_herstellteilmenge_nur_Anzahl = [[this.number]];
        const array_warteliste_eines_teils_zu_herstellteilmenge_nur_Anzahl = new Array;
    ...
    public anzahl_fertigungsauftraege_zu_herstellteil_bearbeitungsliste(wanted: Teil[], wantedfebearbeitend: Fertigungsauftrag[]) {
        ...
        const array_bearbeitungsliste_eines_teils_zu_herstellteilmenge = new Array;
        const array_bearbeitungsliste_eines_teils_zu_herstellteilmenge_nur_Anzahl = new Array;
        ...
        let inhouse_anzeige_array = new Array;
         ...
         wanted.forEach(function(teil){
            ...
            if (teil.nummer === 1 || teil.nummer === 2 || teil.nummer === 3) {
                ...
                const teilenummer_subkomponente = teil.id;
                ...
                zweite_teile_tabelle.forEach(function(teil){
                    ...
                    if (teil.subkomponentes !== null) {
                        const subkomponenten = teil.subkomponentes;

                        /*if (subkomponenten == teilenummer_subkomponente.toString()) {
                            console.log("!!!!! Prüfung hat geklappt");
                            console.log("teilenummer_subkomponente " + teilenummer_subkomponente +" teil.subkomponente: "
                            + subkomponenten.toString());
                            vertriebswunsch_subkomponente = produktionsauftraege;
                            console.log("vertriebswunsch_subkomponente
                            (sollte Produktionsauftaege entsprechen vonübergeordneter Komponente entsprechen):  "
                             + vertriebswunsch_subkomponente);
                        }*/
            ...
            console.log('Produktionsauftraege :' +  produktionsauftraege + 'Vertiebswunsch' + teil.vertriebswunsch
                + 'Sicherheitsbestand' + teil.sicherheitsbestand + 'Bestand_Vorperiode' + teil.istmenge + 'Aufträge Warteschlange'
                + auftraege_wartend_zu_herstellteil + 'Aufträge Bearbeitung' + auftraege_bearbeitend_zu_herstellteil)
